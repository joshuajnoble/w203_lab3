...
```
\
There are several outliers in the variable crmrte and the distribution is right skewed. We have ninety observations so non-normality is not a top concern but this distribution is not perfectly normal. 

The largest outliers on the right side of the distribution are examined. There are no clear 
```{r}
...
```
\
There is an obvious outlier for wser in 84 . The mean services wage across all the counties is $275 ( with a std dev of  206) and 84 has wser of 2177 (~9sd from mean), which seems like a measurement or typographical error. The next highest average weekly wage in any sector is 646 versus the value of 2177.It is very possible that it is an error but we will revisit this later. 
...
```{r}
#plot histograms of just the wage variables
sum(data_crmrte$urban) # There are 8 Urban areas out of 90
...
The backward stepwise regression yielded a more reasonable model so that is the model we are choosing for our balanced model. This model strikes a nice balance between parsimony and explanatory power. The variables included are prbarr, prbconv, polpc, density, pctmin80, and mix. Six out of the original 24 independent variables are included. The adjust r-squared is only 3% lower (77.6% vs. 80.6%). It includes a blend of actionable items for the campaign in the deterrent data as well as demographic variables that perhaps can focus the campaign's efforts.
>>>>>>> d1314924f22776dec7605f71056c9d865dcc35d3
