...
plot(jitter(cleaned_convictions), jitter(crime_data$crmrte), xlab = "Crime Rate", ylab = "Probability of conviction")
...
plot(jitter(crime_data$taxpc), jitter(crime_data$crmrte), xlab = "Crime Rate", ylab = "Tax receipts")
abline(crime_tax)
#summary(crime_tax)
```
...
remove_outliers = function(data, field, min, max) {
  tmp = quantile(data[[field]], probs=c(min, max), na.rm = TRUE)
  return( data[which(crime_data[[field]] > tmp[1] & data[[field]] < tmp[2]),] )
}

#predict crmrte with police presence
outliers_removed_prbarr = remove_outliers(crime_data, 'prbarr', 0.05, 0.95)
...
outliers_removed_polpc = remove_outliers(crime_data, 'polpc', 0.05, 0.95)
...
To see if data became more clear with some of the outliers removed, we can use the Quantile function:
