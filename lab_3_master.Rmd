...
We also we able to find the original panel data set. While we won't use this for prediction, we will use it for data vetting.
...
for (col in 1:ncol(data_crmrte)) {
  for (row in 1:nrow(data_crmrte)) {
      val_orig <- data_crmrte[row, col]
      val_new  <- data_panel_87[row, col]
      if(abs(val_orig-val_new)>.001){
        print(paste("county=",data_crmrte[row, 1], "year=",data_panel_87[row, 2], 
                    "column=",colnames(data_crmrte)[col],"original=", data_crmrte[row, col], 
...
corrdf$pos = (corrdf$cm) + (ifelse(corrdf$cm>0, .05, -.05))
ggplot(data = corrdf,
       aes(y = cm, x=reorder(rownames(corrdf), cm), fill = cm > 0)) + 
       geom_bar(stat = "identity") + 
       geom_text(aes(label=round(cm,2), y=pos), position=position_dodge(width=0.9)) + 
       coord_flip() + 
       theme(legend.position = "none") + ylab("Correlation with log(crmrte)") + xlab("Variable")


```

#TODO josh - make nice correlation graph
