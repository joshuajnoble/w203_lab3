...
One possible explanation is that county 115 could be a fedreal facility or a intelligence facility with national security concerns which explains the highest polpc and avg sentence for crimes. Which might be a barriacaded complex which also explain why the population density is very low and crime rate is the lowest as well since there are not many civilian residents in the area and very low petty crimes.

This county is certainly an outlier, but not one that should be removed from the data.
\
...
- The deterrent variables (prbarr and prbconv) have -ve impact on crime rate indicated by the negative coefficients.
- The demographic variable density and pctmin80 have +ve impact on crime rate
- The demographic variable polpc has an unexpected opposite sign
...
Finally we check the correlation between the X's and the errors in the model to ensure there is no endogeneity in the model. The zero correlation is a necesseary but not sufficient condition for the presence of omitted variable in the regression. There is a more extensive discussion of omitted variable and their implication on model endogeniety in section 5 of the report.
